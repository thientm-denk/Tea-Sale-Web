...
public class AuthenFilter implements Filter {
    ...
    public AuthenFilter() {
        // cac resoujrces ma quyen us duoc phep truy cap
        USER_RESOURCES = new ArrayList<>();
        USER_RESOURCES.add("userPage.jsp");
        USER_RESOURCES.add("checkOut.jsp");
        USER_RESOURCES.add("userCart.jsp");
        USER_RESOURCES.add("viewCart.jsp");
       
        // cac resources ma ad duoc truy cap
        ADMIN_RESOURCES = new ArrayList<>();
        ADMIN_RESOURCES.add("adminPage.jsp");
        ADMIN_RESOURCES.add("createTea.jsp");
        
        // cac resources maf ko can xac thuc pha quyen
        ...
        NON_AUTHEN_RESOURCES.add("index.jsp");
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            for (String o : NON_AUTHEN_RESOURCES) {
                if (uri.contains(o)) {
                    checkContain = true;
                    break;
                }
            ...
            if (checkContain == true) {
                ...
                // xac thuc da dang nhap hay chua
                if (session == null || session.getAttribute("LOGIN_USER") == null) {
                    res.sendRedirect(LOGIN_PAGE);
                } else {
                    UserDTO loginUser = (UserDTO) session.getAttribute("LOGIN_USER");
                    String roleID = loginUser.getRoleID();
                    // cho nay phan quyen dang nhap voi role nnao thi duoc cai do
                    if (AD.equalsIgnoreCase(roleID) && ADMIN_RESOURCES.contains(resource)) {
                        chain.doFilter(request, response);
                    } else if (US.equalsIgnoreCase(roleID) && USER_RESOURCES.contains(resource)) {
                        chain.doFilter(request, response); 
                    }else{
                        res.sendRedirect(LOGIN_PAGE);
                    }
